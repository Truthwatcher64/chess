...
public class StandardAPITests {
    ...
    public void uniqueAuthorizationTokens() throws TestException {
        ...
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
                "Server response code was not 200 OK");
        Assertions.assertNotNull(loginOne.authToken, "Login result.result did not contain an authToken");

        TestModels.TestLoginRegisterResult loginTwo = serverFacade.login(loginRequest);
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
                "Server response code was not 200 OK");
        Assertions.assertNotNull(loginTwo.authToken, "Login result.result did not contain an authToken");
    ...
    public void badAuthCreate() throws TestException {
        ...
        Assertions.assertTrue(createResult.message.toLowerCase(Locale.ROOT).contains("error"),
                "Bad result.result did not return an error message");
        Assertions.assertNull(createResult.gameID, "Bad result.result returned a game ID");
    }
    ...
    public void clearData() throws TestException {
        ...
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
                "Server response code was not 200 OK");
        Assertions.assertEquals(0, listResult.games.length, "list result.result did not return 0 games after clear");
    }
